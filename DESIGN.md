...
- Ships a curated rule pack (`rules/builtin.yar`, 14 rules) covering
  classic injection idioms ("ignore previous instructions",
  "disregard the above"), **OpenAI ChatML** role hijacks
  (`<|im_start|>`, `<|im_end|>`, `<|system|>`, `<|user|>`,
  `<|assistant|>`), **Llama 2 / Mistral** (`[INST]`, `[/INST]`,
  `<<SYS>>`, `<</SYS>>`), **Llama 3** (`<|begin_of_text|>`,
  `<|start_header_id|>`, `<|end_header_id|>`, `<|eot_id|>`),
  **GPT family** (`<|endoftext|>`, `<|fim_prefix|>`, `<|fim_middle|>`,
  `<|fim_suffix|>`), **Claude / Anthropic** (`\n\nHuman:`,
  `\n\nAssistant:`), **Gemini / Gemma** (`<start_of_turn>`,
  `<end_of_turn>`), Alpaca instruction markers (`### Instruction:`),
  system-prompt spoofs (`system prompt:`, `system message:`,
  `[system]`), jailbreak preambles ("DAN mode", "developer mode
  enabled"), exfiltration vocabulary, and tool-call spoofs
...
- Each rule carries `severity`, `confidence`, and `message` metadata
  ...
  the `Finding`.
- **Three-pass scanning**: each chunk is scanned three times — once
  as-is (precise byte spans), once after stripping U+0300–U+036F
  Latin combining marks (defeats zalgo / strikethrough / underline
  obfuscation), and once after rewriting Cyrillic / Greek confusables
  to their Latin equivalents (defeats homoglyph substitution).
  Findings from the second and third passes carry a `(denoised)` /
  `(deconfused)` suffix in the message and use the chunk-level span
  because the rewriting changes string lengths. The denoise /
  deconfuse passes are skipped when their output is identical to the
  primary text, so plain-ASCII inputs cost nothing extra.
...
- **Invisible characters** in five categories plus combining-mark
  obfuscation:
...
- **Combining-mark obfuscation**:
  - **Stacked combining marks** (High): any base character with
    ≥ 2 stacked U+0300–U+036F combining marks. This is the zalgo
    glyph pattern. After NFKC, real precomposed Vietnamese / French /
    German / Spanish text has at most one mark per base, so the
    detector does not flag normal accented prose (`café`, `résumé`,
    `Hà Nội`).
  - **Combining overlay marks** (Medium): ≥ 5 strikethrough (U+0336)
    or underline (U+0332) combining marks per chunk. Parseltongue's
    overlay-style transforms apply one mark per letter rather than
    stacking them, so the count threshold catches them where the
    stack-depth threshold cannot.
...
End-user instructions for enabling this detector (build flags +
TOML config + bundled-model fetch) live in the README under
[Using the optional features](README.md#using-the-optional-features).
...
tldrsec. End-user instructions for enabling this detector (build
flags, TOML config, API-key env var) live in the README under
[Using the optional features](README.md#using-the-optional-features).
...
### 6.1 System build requirements

The default-feature build needs only a working C compiler in addition
to a Rust toolchain — `tree-sitter` grammars and `zstd-sys` (pulled in
by `yara-x`) compile small amounts of C source via `cc-rs`. On Linux
that means `build-essential` (Debian / Ubuntu) or the `gcc` package
group (Fedora / Arch); macOS ships one with Xcode command-line tools;
Windows uses MSVC build tools.

The optional **`embeddings`** feature has additional system
requirements on Linux: the `ort-sys` crate's build script downloads
the ONNX Runtime native library at compile time using a `ureq`
build that links `native-tls` → `openssl-sys`, which needs
`pkg-config` and the OpenSSL development headers. On Debian / Ubuntu
/ WSL: `sudo apt-get install -y pkg-config libssl-dev`. On Fedora /
RHEL: `sudo dnf install -y pkgconf-pkg-config openssl-devel`. On
Arch: `sudo pacman -S pkgconf openssl`. macOS Homebrew users need
the `pkg-config` and `openssl` formulas. The `pdf` and `llm` features
have no extra system requirements beyond a C compiler.
...
- **90 tests** total, split across:
  - **64 unit tests** (in-module `#[cfg(test)]` blocks) covering
